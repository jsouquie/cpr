...
public:
   ...
   void PerformDetached(Then&& then, cpr::Session&& session /* borrowed */)
   {
      ...
      Submit(std::unique_ptr<ReqP>{ new Req(std::forward<Then>(then), std::move(session)) });
   }
...
private:
   ...
   struct TReqP : public ReqP
   {
      ...
      TReqP(Then&& then, cpr::Session&& session) : m_then(std::move(then)) { m_session = std::move(session); m_completeFn = &complete; }
      TReqP(const Then& then, cpr::Session&& session) : m_then(then) { m_session = std::move(session); m_completeFn = &complete; }
   ...
   struct TReqPNCB : public ReqP
   {
      ...
      TReqPNCB(Then&& then, cpr::Session&& session) : m_then(std::move(then)) { m_session = std::move(session); m_completeFn = &complete; }
      TReqPNCB(const Then& then, cpr::Session&& session) : m_then(then) { m_session = std::move(session); m_completeFn = &complete; }
   ...
   struct TReqSession : public ReqP
   {
      ...
      TReqSession(Then&& then, cpr::Session&& session) : m_then(std::move(then)) { m_session = std::move(session); m_completeFn = &complete; }
      TReqSession(const Then& then, cpr::Session&& session) : m_then(then) { m_session = std::move(session); m_completeFn = &complete; }
